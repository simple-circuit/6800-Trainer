...
//      0xC00 reads 0xFF for no key down else 0x00 to 0x0F or 0x20 or 0x10 for pressed key
//      0xC06 returns key column-1, low bits indicate  b5-0 b4-1 b3-4 b2-7 b1-A b0-D
//      0xC05 returns key column-2, low bits indicate  b5-grey b4-2 b3-5 b2-8 b1-B b0-E
//      0xC03 returns key column-3, low bits indicate  b5-red b4-3 b3-6 b2-9 b1-C b0-F
//
...
void load_boot_rom(void){
 ...
 else for ( i = 0; i < 0x490; i++) MITS_RAM[0xFB70 + i] = TRAINER[i];
}
...
void setup1() { 
   ...
   pinMode(12, OUTPUT); //*CS      old-clock
   pinMode(13, OUTPUT); //data
   pinMode(25, OUTPUT); //LED
   delay(2000);
   gpio_put(12,1); //set up clock and enable levels
   gpio_put(11,0);
   header7seg(0x0c);  //enable display
   data7seg(1);
   end7seg();
   ...
   data7seg(1);
   end7seg();
   header7seg(0x09);  //decode mode
   data7seg(0);
   end7seg();
   header7seg(0x0b);
   data7seg(7);      //scan all eight displays
   end7seg();
   header7seg(0x0f);
   data7seg(0);      //turn off display test
   end7seg();
}
...
while (true){ 
 ...
 update_display();
 pinMode(10, OUTPUT); //10 3 6 9 C F
 gpio_put(10,0); 
 delayMicroseconds(500);
...
void update_display(void){
 ...
 for (i=0; i<=7; i++){
  ...
  if (k !=0){
    m = 0xC100 + ((8-i)<<4);
    for (j=0; j<=7;j++){
      MITS_RAM[m+j] = ((ascii[k & 0x7f] | (k & 0x80))>>j)&1;
    }
    if ((MITS_RAM[i+0xc100] & 0x80)!=0) MITS_RAM[m+7] = 0x01;
    MITS_RAM[i+0xc100] = 0;
  }
 }
 ...
 for (j=1;j<=8;j++){
   header7seg(j) ; 
   for (i=(0xc107+(j<<4)); i>=(0xc100+(j<<4)); i--) {
    delayMicroseconds(1);
    gpio_put(13,MITS_RAM[i] & 1);
    delayMicroseconds(1);
    gpio_put(11,1);
    delayMicroseconds(1);
    gpio_put(11,0);  
 ...
 if (MITS_RAM[0xC108] != 0) gpio_put(25,1); else gpio_put(25,0); 
}

void data7seg(unsigned int data){
  for (int i=7; i>=0; i--) {
   delayMicroseconds(1);
   gpio_put(13, (data>>i) & 1);
   delayMicroseconds(1);
   gpio_put(11,1);
   delayMicroseconds(1);
   gpio_put(11,0);  
...
void header7seg(unsigned int address){
 gpio_put(13,0);
 delayMicroseconds(1);
 gpio_put(12,0);
 delayMicroseconds(1);
 ...
 gpio_put(11,0); //0
 delayMicroseconds(1);
 gpio_put(13,(address>>3) & 1);
 delayMicroseconds(1);
 gpio_put(11,1);
 delayMicroseconds(1);
 gpio_put(11,0);      //b3
 delayMicroseconds(1);
 gpio_put(13,(address>>2) & 1);
 delayMicroseconds(1);
 gpio_put(11,1);
 delayMicroseconds(1);
 gpio_put(11,0);     // b2
 delayMicroseconds(1);
 gpio_put(13, (address>>1) & 1);
 delayMicroseconds(1);
 gpio_put(11,1);
 delayMicroseconds(1);
 gpio_put(11,0);    // b1
 delayMicroseconds(1);
 gpio_put(13,(address) & 1);
 delayMicroseconds(1);
 gpio_put(11,1);
 delayMicroseconds(1);
 gpio_put(11,0);   // b0  
